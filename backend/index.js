...
app.post('/addAttendence',async (req,res)=>{
    ...
    if(existingStudent)
    {
        existingStudent.totalPresent = existingStudent.totalPresent + 1;
        await existingStudent.save();
        console.log(existingStudent.name);
        console.log(existingStudent);
        res.json({message : "student presente",name : existingStudent.name,rollNumber : existingStudent.rollNumber});
    }
    else 
    {
        return res.status(404).json({message : "student not found"});
